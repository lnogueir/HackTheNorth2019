...
export default class HTN_app extends React.Component {
  ...
  speak = async (endpoint = '', message = '') => {
    ...
    if (this.state.word == null) {
      const { sound: soundObject, status } = await Audio.Sound.createAsync(
        { uri: SERVER_ADDRESS + param, headers: { 'Cache-Control': 'no-cache' } },
        { shouldPlay: true });
    } else {
      const { sound: soundObject, status } = await Audio.Sound.createAsync(
        { uri: SERVER_ADDRESS + endpoint + '?prediction=' + this.state.word + '&language=' + this.state.selected_language, headers: { 'Cache-Control': 'no-cache' } },
        { shouldPlay: true });
    }
  }
  ...
  get_languages = async () => {
    let req_url = SERVER_ADDRESS + '/get_languages'
    try {
      fetch(req_url)
  ...
  send_photo = async (endpoint, data) => {
    let req_url = SERVER_ADDRESS + endpoint
    ...
    if (this.state.photo_mode) {
      ...
      return (
        <View style={{ flex: 1, margin: 40, alignItems: 'center', height: '100%', flexDirection: 'column' }}>
          ...
          <View style={{ marginTop: 25, margin: 10, flexDirection: 'row', justifyContent: 'space-evenly' }}>
            ...
            <Button borderRadius={90} icon={<Ionicons
              name="md-camera"
              size={35}
              color="black"
            />}
      ...
      return (
        <View style={{ flex: 1, padding: 20, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Button icon={<Ionicons name='md-home' color='white' size={30} />} buttonStyle={{ marginTop: 50, backgroundColor: '#1968e8' }} title='Back to home' onPress={() => { this.setState({ word: null, is_result_page: false, home_mode: true }) }} />
        </View>
      )
    }
    else {
